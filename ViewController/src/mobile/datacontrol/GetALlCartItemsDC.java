...
public class GetALlCartItemsDC {
    ...
    public void createOrder() {
        ...
        ve = AdfmfJavaUtilities.getValueExpression("#{pageFlowScope.orderTypeLov}", String.class);
        orderTypeId = ((String) ve.getValue(AdfmfJavaUtilities.getELContext())).trim();

        if (orderTypeId != null && !("".equals(orderTypeId)))
            System.out.println("createOrder: - orderTypeId String is :" + orderTypeId.toString());

        else {
//            System.out.println("createOrder: - orderTypeId == null");
//            GetOrderTypeLOVDC obj = new GetOrderTypeLOVDC();
//            obj.getCurrentOrderId();
//            ve = AdfmfJavaUtilities.getValueExpression("#{pageFlowScope.getIdforOrderType}", String.class);
//            orderTypeId = ((String) ve.getValue(AdfmfJavaUtilities.getELContext()));
            orderTypeId = "1020";
            System.out.println("createOrder: - orderTypeId == getIdforOrderType->" + orderTypeId);
        }


        //        Number orderTypeIdTypeCasted = Integer.parseInt(orderTypeId);
        //        System.out.println("createOrder: - orderTypeIdTypeCasted is :"+orderTypeIdTypeCasted);
        ...
        String payload =
            "{\n"+"\"P_ORDER_HEADER\": {\"USER_ID\" : \"" + userName + "\",\"ORG_ID\": \"" + orgId + "\",\"ORDER_TYPE_ID\": \"" +
            orderTypeId + "\",\"BILL_TO\": \"" + billToAccNum + "\",\"SHIP_TO\": \"" + shipTpAccNum +
            "\",\"COUPON\": \"" + coupon + "\",\"ATTRIBUTE1\": \"" + null + "\",\"ATTRIBUTE2\": \"" + null +
        ...
        for (int i = 0; i < s_getItemFromCart.size(); i++) {
            ...
            payload =
                payload + "{\"INVENTORY_ITEM_ID\":\"" + s_getItemFromCart.get(i).getATTRIBUTE1() +
                "\",\"PRICE_LIST_ID\": \"" + s_getItemFromCart.get(i).getPRICE_LIST() + "\",\"ORDERED_QUANTITY\": \"" +
                s_getItemFromCart.get(i).getITEM_QUANTITY() + "\",\"ATTRIBUTE1\": \"" + "\"" + ",\"ATTRIBUTE2\": \"" +
